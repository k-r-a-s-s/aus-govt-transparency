#!/usr/bin/env python
"""
Analyze potential double disclosure entities using Google's Gemini AI.

This script processes batches of potential double disclosure entities identified in the 
Australian Government Transparency dataset. It uses Google's Gemini AI to determine whether 
each entity represents multiple separate entities that should be split (true double disclosures) 
or a single organization with a compound name.

The script:
1. Loads batches of entities containing separators like "and", "&", ",", etc.
2. Sends these entities to Gemini AI with context from their disclosures
3. Interprets Gemini's analysis of whether each entity is a SINGLE or MULTIPLE entity
4. Compiles and saves the results for further processing

Results are used to improve data accuracy by splitting true double disclosures into 
...
class GeminiDoubleDisclosureAnalyzer:
    """
    Analyze potential double disclosure entities using Gemini AI.
    
    This class uses Google's Gemini AI to determine whether entities in the Australian 
    Government Transparency dataset containing separators like "and", "&", etc. represent:
    1. True double disclosures (multiple entities incorrectly combined)
    2. Single entities with compound names
    
    The analysis considers Australian context and naming conventions, and provides confidence 
    levels for each classification to assist with manual review.
    """
    
    def __init__(
        self,
        input_dir: str = "scripts/gemini_batches",
        ...
        """
        Initialize the double disclosure analyzer.
        
        Args:
            input_dir: Directory containing batch files of potential double disclosure entities
            output_dir: Directory to save analysis results to
            api_key: Google AI API key (direct parameter)
            api_key_file: Path to a file containing the Google AI API key
...
def main():
    """
    Main entry point for the script to analyze double disclosure entities.
    """
    parser = argparse.ArgumentParser(description="Analyze potential double disclosure entities using Gemini to distinguish between true multiple entities and single organizations with compound names")

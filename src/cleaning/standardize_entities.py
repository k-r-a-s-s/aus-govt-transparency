"""Entity Normalization and Abbreviation Expansion Script for Disclosure Database

This script performs two main functions:
1.  Abbreviation Expansion: It uses a provided JSON mapping file to replace 
    known abbreviations in the 'split_entity' column with their full names.
2.  Entity Normalization: It applies basic normalization (lowercase, remove 
    non-alphanumeric/space/& characters, collapse whitespace) to the 
    (potentially expanded) entity name.

The result is saved to a new 'normalized_entity' column.

This normalized column can then serve as input for more advanced processes 
like graph-based fuzzy grouping.

Requires a JSON file mapping abbreviations (keys) to full names (values).
"""

import sqlite3
import re
import argparse
import time
import json
from typing import List, Tuple, Dict

def normalize(name: str) -> str:
    """Apply basic normalization to a name."""
    if not name:
        return ""
    # Normalization happens *after* potential expansion
    name = name.casefold().strip() 
    # Keep alphanumeric, whitespace, and ampersand. Remove others.
    name = re.sub(r"[^\w\s&]", "", name)
    # Collapse multiple whitespace characters into a single space.
    name = re.sub(r"\s+", " ", name)
    return name.strip()

def expand_abbreviation(name: str, mapping: Dict[str, str]) -> str:
    """Expand abbreviation if found in the mapping (case-insensitive key lookup)."""
    if not name:
        return ""
    # Check against lowercase version of the name as keys are lowercase in the JSON
    return mapping.get(name.casefold(), name) 

def get_db_connection(db_path, max_retries=3, timeout=60):
    """Get a database connection with retry mechanism."""
    for attempt in range(max_retries):
        try:
            conn = sqlite3.connect(db_path, timeout=timeout)
            # Improve performance and reliability
            conn.execute("PRAGMA journal_mode=WAL;") 
            conn.execute("PRAGMA synchronous=NORMAL;")
            return conn
        except sqlite3.OperationalError as e:
            if "database is locked" in str(e).lower() and attempt < max_retries - 1:
                print(f"Database locked, retrying attempt {attempt + 2}/{max_retries}...")
                time.sleep(1)  # Wait 1 second before retrying
                continue
            raise
    raise sqlite3.OperationalError(f"Failed to connect to database '{db_path}' after {max_retries} attempts.")

def ensure_normalized_entity_column(db_path: str) -> None:
    """Ensure the normalized_entity column exists in the disclosures table."""
    print(f"Checking for 'normalized_entity' column in {db_path}...")
    conn = None
    try:
        conn = get_db_connection(db_path)
        cursor = conn.cursor()
        
        cursor.execute("PRAGMA table_info(disclosures)")
        columns = [col[1] for col in cursor.fetchall()]
        
        if 'normalized_entity' not in columns:
            print("Adding 'normalized_entity' column...")
            cursor.execute("ALTER TABLE disclosures ADD COLUMN normalized_entity TEXT")
            # Add an index for faster lookups if needed later
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_normalized_entity ON disclosures(normalized_entity)")
            conn.commit()
            print("'normalized_entity' column added and indexed.")
        else:
            print("'normalized_entity' column already exists.")
            
    except Exception as e:
        print(f"Error ensuring 'normalized_entity' column exists: {e}")
        # Optionally re-raise if it's critical
    finally:
        if conn:
            conn.close()

def main():
    parser = argparse.ArgumentParser(description='Normalize entity names from split_entity column.')
    parser.add_argument('--db', required=True, help='Path to the SQLite database')
    parser.add_argument('--mapping', required=True, help='Path to the JSON file containing entity abbreviations mapping')
    args = parser.parse_args()

    # 0. Load the abbreviation mapping
    try:
        with open(args.mapping, 'r') as f:
            entity_mapping: Dict[str, str] = json.load(f)
            # Convert keys to lowercase for consistent matching
            entity_mapping = {k.casefold(): v for k, v in entity_mapping.items()} 
        print(f"Successfully loaded {len(entity_mapping)} entity mappings from {args.mapping}")
    except FileNotFoundError:
        print(f"Error: Mapping file not found at {args.mapping}")
        return
    except json.JSONDecodeError:
        print(f"Error: Could not decode JSON from {args.mapping}. Please ensure it's valid JSON.")
        return
    except Exception as e:
        print(f"An error occurred loading the mapping file: {e}")
        return

    # 1. Ensure the target column exists
    ensure_normalized_entity_column(args.db)

    conn = None
    try:
        # 2. Connect to the database
        conn = get_db_connection(args.db)
        cursor = conn.cursor()
        
        # 3. Fetch distinct non-null split_entity values
        print("Fetching distinct split entities...")
        cursor.execute("SELECT DISTINCT split_entity FROM disclosures WHERE split_entity IS NOT NULL AND split_entity != ''")
        original_entities: List[Tuple[str]] = cursor.fetchall()
        print(f"Found {len(original_entities)} unique split entities to process.")

        if not original_entities:
            print("No split entities found to process.")
            return

        # 4. Expand and Normalize the entities
        print("Expanding abbreviations and normalizing entities...")
        update_data: List[Tuple[str, str]] = []
        processed_count = 0
        for (original_entity,) in original_entities:
            # First, try to expand the abbreviation
            expanded_name = expand_abbreviation(original_entity, entity_mapping)
            # Then, normalize the (potentially expanded) name
            normalized_name = normalize(expanded_name) 
            
            # We still use the original_entity in the WHERE clause for the update
            update_data.append((normalized_name, original_entity))
            processed_count += 1
            if processed_count % 1000 == 0:
                 print(f"  Processed {processed_count}/{len(original_entities)}...")

        # 5. Update the database
        if update_data:
            print(f"\nApplying {len(update_data)} normalization updates to the 'normalized_entity' column...")
            try:
                # Use executemany for potentially faster bulk updates
                cursor.executemany("""
                    UPDATE disclosures 
                    SET normalized_entity = ? 
                    WHERE split_entity = ?
                """, update_data)
                
                # Commit the changes
                conn.commit()
                print(f"Successfully updated {cursor.rowcount} rows.") # Reports total rows affected across all operations in executemany
            except sqlite3.Error as e:
                print(f"Database error during update: {e}")
                conn.rollback() # Roll back changes on error
        else:
            print("No normalization changes needed or no entities to process.")

    except sqlite3.Error as e:
        print(f"An error occurred: {e}")
    finally:
        if conn:
            conn.close()
            print("Database connection closed.")

if __name__ == '__main__':
    main()